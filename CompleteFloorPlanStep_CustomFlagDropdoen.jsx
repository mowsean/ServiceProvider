...
export default function CompleteFloorPlanPPStep({
  ...
  //   <select
  //     className="block w-1/6 h-11	 min-w-0 sm:text-xs"
  //     {...rest}
  //     value={value || 'ZZ'}
  ...
  const CountrySelect = ({ value, onChange, labels, ...rest }) => (
    <div className="relative inline-block text-left">
      <button
        ...
        type="button"
        className="inline-flex justify-center w-46 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-md hover:bg-gray-50 focus:outline-none focus:border-blue-300 focus:ring focus:ring-blue-200 active:bg-gray-200"
      ...
      {/* {isCountryOpen && (
        <div className="absolute right-0 z-10 mt-2 w-56 origin-top-right rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
          ...
          {getCountries().map((country) => (
            <li key={country} country={country} className="blockx px-4 py-2 text-sm text-gray-700 hover:bg-blue-500 hover:text-white cursor-pointer" onClick={onChangeCountry}>
              {getCountryFlag(country)} {labels[country]} +{getCountryCallingCode(country)}
            </li>
          ))}
        </div>
      ...
      {isCountryOpen && (
        ...
        <ul className="absolute1 z-10 h-48 w-80 origin-top-right overflow-auto	scroll-smooth bg-white border border-gray-300 rounded-md shadow-lg">
          {getCountries().map((country) => (
    ...
    // <>
    //   <button id="dropdownDefaultButton" data-dropdown-toggle="dropdown" className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 font-medium text-sm px-2 py-3 text-center inline-flex items-center" type="button">
    //     {getCountryFlag('us')}
    ...
    //       {getCountries().map((country) => (
    //         <li key={country}>
    //           <a href="#" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">{getCountryFlag(country)} {labels[country]} +{getCountryCallingCode(country)}</a>
    //         </li>
    //       ))}
    //     </ul>
    //   </div>
              ...
              <div className="space-y-4 divide-y divide-gray-200">
                ...
                <div className="mt-6 grid grid-cols-1 lg:grid-cols-2 gap-y-6 gap-x-4 border-none">
                  ...
                  <div className="col-span-1">
                    ...
                    <div className="mt-1 w-full rounded rounded-md shadow-sm">
                      <PhoneInput
                        countrySelectComponent={renderFlagDropdown}
                        initialValueFormat='national'
                        onBlur={handleInputBlur}
                        value={phoneNo}
                        className={'flex-1 block w-full min-w-0 rounded sm:text-sm border-red-500'}
